...
描述"发生了什么错误"，而不是"操作失败"
  - ✅ `ResourceLoadException`、`SceneInitializeException`
  - ❌ `LoadResourceFailedException`、`InitializeSceneFailedException`
...
public class ResourceLoadException : Exception
{
    ...
    public ResourceLoadException(string assetPath)
        : base($"Failed to load resource at path: {assetPath}")
    {
        AssetPath = assetPath;
    }
}
...
public class ResourceLoadedEventArgs : EventArgs
{
    ...
    public ResourceLoadEventArgs(string assetPath, UnityEngine.Object asset)
    {
        AssetPath = assetPath;
        Asset = asset;
    }
