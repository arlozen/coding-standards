...
当派生成员（重写属性、重载方法、接口实现、抽象方法实现）的行为与基类或接口定义完全一致时，应使用 `/// <inheritdoc/>` 继承注释，避免重复。
...
当方法有多个重载时，应在主要版本添加完整注释，其他重载可使用 <inheritdoc/> 引用主要版本。
...
实现接口成员时，如果行为与接口定义一致，使用 <inheritdoc/>；如果实现有特定行为，添加额外说明。
...
public class PlayerData : ISerializable
{
    /// <inheritdoc/>
    /// <remarks>
    /// This implementation uses Newtonsoft.Json with indented formatting.
    /// </remarks>
...
当派生成员行为与基类部分不同时，继承基类注释并补充说明差异。

```csharp
public class AdvancedEventManager : BaseEventManager
{
    /// <inheritdoc/>
    /// <remarks>
    /// This override adds thread-safety using a <see cref="ReaderWriterLockSlim"/>.
    /// All event operations are now thread-safe.
    /// </remarks>
...
1. **优先使用 `<inheritdoc/>`**：避免复制粘贴注释，减少维护成本
2. **明确行为变更**：当实现有特殊行为时，在 `<remarks>` 中说明
3. **保持一致性**：同一类中的派生成员使用统一的注释风格
